...
export const TIMELINE: TimelineMonth[] = [
  {
    ...
    entries: [
      ...
      },
      {
        ...
        sub: '21 days of moves in a row.',
        impact: 'Three weeks straight is where the habit stops needing willpower.',
        xp: 50,
        onStreak: true,
        icon: '✦',
        daysAgo: 12,
    ...
    entries: [
      {
        ...
        sub: 'Idle cash finally earning real interest.',
      ...
      },
      {
        ...
        sub: 'Assets minus debts, moving the right way.',
        impact: 'The first milestones are the slowest — compounding accelerates from here.',
        onStreak: true,
        isPinned: true,
        icon: '◉',
        daysAgo: 41,
...
function buildPlaidEntries(accounts: any[], transactions: any[]): TimelineEntry[] {
  ...
  if (crossed) {
    entries.push({
      ...
      sub: 'A real threshold, from your connected accounts.',
  ...
  entries.push({
    ...
    impact: 'Your score, trajectory, and cohort ranking are now computed from your actual finances.',
    icon: '◈',
    daysAgo: 0,
  });
